...
export const options = {
  ...
  thresholds: {
    ...
    // Error rate must be < 1% (only count non-200 status codes, not empty results)
    'error_rate': ['rate<0.01'],
    // HTTP errors must be < 1% (only count actual HTTP errors, not empty results)
...
// Test directly against SearchService (port 5002) instead of API Gateway
...
// Direct to SearchService: GET /api/search?query=python
function performShortSearch() {
  ...
  // Log non-200 responses for debugging (only first few to avoid spam)
  if (response.status !== 200 && Math.random() < 0.01) {
    console.warn(`Non-200 response: ${response.status} for query "${query}". Body: ${response.body.substring(0, 200)}`);
  }
  ...
  const success = check(response, {
    ...
    'short search has valid JSON': (r) => {
      if (r.status !== 200) return true; // Don't validate JSON for error responses
      try {
    ...
    'short search has results structure': (r) => {
      if (r.status !== 200) return true; // Don't validate structure for error responses
      try {
        const body = JSON.parse(r.body);
...
// Direct to SearchService: GET /api/search?query=python programming&page=1&pageSize=20&sortBy=price
function performComplexSearch() {
  ...
  const url = `${BASE_URL}/api/search?query=${encodeURIComponent(query)}&page=${page}&pageSize=${pageSize}&sortBy=${sortBy}`;
...
export function setup() {
  console.log(`Starting DIRECT search load test against ${BASE_URL}`);
  ...
  console.log('NOTE: Testing directly against SearchService, bypassing API Gateway');
  
  // Verify SearchService is accessible
  ...
  if (healthCheck.status !== 200) {
    console.warn(`Warning: SearchService health check returned status ${healthCheck.status}`);
    console.warn('Test will continue, but SearchService may not be fully operational');
  } else {
    console.log('✓ SearchService health check passed');
  }
...
export function teardown(data) {
  console.log(`Direct search load test completed for ${data.baseUrl}`);
